...
function instruction(num) {
	var str = String(document.getElementById("myinput").value); //获得当前显示数据 
	str = (str != "0") ? ((operate == 0) ? str : "") : ""; //如果当前值不是"0"，且状态为0，则返回当前值，否则返回空值; 
	str = str + String(num); //给当前值追加字符 
	if (str.length < 22) {
		document.getElementById("myinput").value = str; //刷新显示
		operate = 0; //重置输入状态 
		quit = 0; //重置防止重复按键的标志 
	}
}
...
function doublezero() {
	...
	document.calculator.myScreen.value = str;
	operate = 0;
}
...
// 
function calculate() {
	numshow = Number(document.getElementById("myinput").value);
	...
	document.getElementById("myinput").value = numshow;
	num = result; //存储当前值 
}

function clearnote() { //清空提示 
	document.getElementById("note").innerHTML = "欢迎使用";
}